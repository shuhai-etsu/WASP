...
/**
 * Class: CreateGenderRequest
 *
 * Purpose: Class is used to validate a new gender entry based upon a defined set of rules. The class is called by the
 *          GenderController when a user attempts to add a new gender to the system.
 *
...
class CreateGenderRequest extends FormRequest
{
    ...
    public function rules()
    {
        return
        [
            'abbreviation'  => 'required|min:1|unique:genders,abbreviation',
            'description'   => 'required|min:1|unique:genders,description',
            'comment'       => 'max:255'
        ];
    }
}
