 ...
 *
 * Purpose: Class is used to validate a new relationship entry based upon a defined set of rules. The class is called by
 *          the RelationshipController when a user attempts to add a new relationship to the system.
 *
...
class CreateRelationshipRequest extends FormRequest
{
    ...
    {
        return [
            'abbreviation'  => 'required|min:1|unique:relationships,abbreviation',
            'description'   => 'required|min:1|unique:relationships,description',
            'comment'       => 'max:255'
        ];
    }
}
