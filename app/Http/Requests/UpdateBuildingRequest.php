 ...
 *
 * Purpose: Class is used to validate an existing building entry based upon a defined set of rules. The class is
 *          called by the BuildingController when a user attempts to update an existing building.
 *
 * @todo remove from code base, since buildings won't be tracked?  or, at least, move to a folder for deprecated code.
 *
 * @see BuldingController
 *
...
class UpdateBuildingRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            //==========================================================================================================
            'id'                => 'exists:buildings,id,id,' . $this->get('id'),
            'abbreviation'      => 'required|min:1|max:10|unique:buildings,abbreviation,' . $this->get('id'),
            'description'       => 'required|min:1|max:100|unique:buildings,description,' . $this->get('id'),
            'address1'          => 'required|min:1|max:100',
            'address2'          => 'max:100',
            'city'              => 'required|min:1:max25',
