 ...
 *
 * Purpose: Class is used to validate a new state entry based upon a defined set of rules. The class is called by the
 *          StateController when a user attempts to add a new state to the system.
 *
 * @see StateController
 *
...
class CreateStateRequest extends FormRequest
{
    ...
    {
        return [
            'abbreviation'  => 'required|min:1|unique:states,abbreviation',
            'description'   => 'required|min:1|unique:states,description',
            'comment'       => 'max:255'
        ];
    }
}
