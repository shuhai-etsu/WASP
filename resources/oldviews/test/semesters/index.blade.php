...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function ()
        {
            ...
            {
                var path = "{!! url('/semesters/') !!}" + "/" + JSON.parse(data)[row].id;
            ...
            var startDateRenderer = function(row)
            {
                return formatDate(JSON.parse(data)[row].start_date);
            }

            var endDateRenderer = function(row)
            {
                return formatDate(JSON.parse(data)[row].end_date);
            }
            ...
            (
                    {
                        ...
                        columns: [{ text: 'ID', datafield: 'id', width: 25, hidden:true },
                                  { text: 'Abbreviation', datafield: 'abbreviation', width: 230 },
                                  { text: 'Description', datafield: 'description', width: 210 },
                                  { text: 'Start Date', datafield: 'start_date', cellsrenderer: startDateRenderer, width: 110 },
                                  { text: 'End Date', datafield: 'end_date', cellsrenderer: endDateRenderer,width: 110 },
            ...
            function formatDate(date)
            {
                if(date)
                {
                    ...
                    return '<span style="margin: 4px; margin-top: 5px; float: left;">' + date + '</span>';
                }
                else
                {
                    return "";
            ...
            $("#jqxgrid").bind('rowclick', function (event)
            {
                ...
                if (click - lastClick < 300)
                {
                    if (lastRow == event.args.rowindex)
                    {
                        ...
                        window.location.assign('/classrooms/' + datarow.id + '/edit');
                    }
                }
                lastClick = new Date();
