...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function ()
        {
            ...
            {
                var path = "{!! url('/security_privilege_types/') !!}" + "/" + JSON.parse(data)[row].id;
            ...
            $("#jqxgrid").bind('rowclick', function (event)
            {
                ...
                if (click - lastClick < 300)
                {
                    if (lastRow == event.args.rowindex)
                    {
                        ...
                        window.location.assign('/security_privilege_types/' + datarow.id + '/edit');
                    }
                }
                lastClick = new Date();
