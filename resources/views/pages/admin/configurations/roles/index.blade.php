...
@section('page')
        ...
        @section('table_rows')
            @foreach($data as $row)
                <tr>
                    ...
                    <td>{{$row->comment}}</td>
                    <td>
                        @include('template.table_button', array('button_link'=> URL::to('roles/'.$row->id.'/edit')))
                    </td>
                </tr>
            @endforeach
        @stop
    ...
    <div class="row">
        <a id="roles_id" class="btn btn-primary pull-left" href="{{URL::to('roles/create')}}" style="position:relative;bottom: 34px;">New Role</a>
    </div>
    <br/>
