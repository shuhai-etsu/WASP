...
@section('page')
        ...
        @section('table_rows')
            @foreach($data as $row)
                <tr>
                    ...
                    <td>{{$row->comment}}</td>
                    <td>
                        @include('template.table_button', array('button_link'=> URL::to('degree_types/'.$row->id.'/edit')))</td>
                </tr>
            @endforeach
        @stop
    ...
    <div class="row">
        <a id="degree_id" class="btn btn-primary pull-left" href="{{URL::to('degree_types/create')}}">New Degree</a>
    </div>
    <br/>
