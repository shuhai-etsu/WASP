...
@section('page')
        ...
        @section('table_rows')
            @foreach($data as $row)
                <tr>
                    ...
                    <td>{{$row->comment}}</td>
                    <td>
                        @include('template.table_button', array('button_link'=> URL::to('certification_types/'.$row->id.'/edit')))
                    </td>
                </tr>
            @endforeach
        @stop
    ...
    <div class="row">
        <a id='certification_types_id' class="btn btn-primary pull-left" href="{{URL::to('certification_types/create')}}" style="position:relative;bottom: 34px;">New Certification</a>
    </div>
    <br/>
