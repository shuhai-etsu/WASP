...
/**
 * Class: CreateFinancialAidTypesTable
 *
 * Purpose: The class is used create and drop the financial_aid_types table, which stores information on the types of
 *          financial aid a worker supported by the application that a worker is receiving
 *
...
class CreateFinancialAidTypesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('financial_aid_types', function (Blueprint $table)
        {
            ...
            $table->string('abbreviation', 10)
                  ->unique()
                  ->comment = 'Abbreviation of a given financial aid type, such as APS for 
                               Academic Performance Scholarship.';

            $table->string('description', 100)
                  ->unique()
                  ->comment = 'Description of a given financial aid type entry, such as Academic 
                               Performance Scholarship.';

            $table->unsignedInteger('max_hours')
                ->comment = 'Total number of hours student worker with a particular financial aid 
                                is permitted to work per week.';

            $table->string('comment', 255)
                  ->nullable()
                  ->comment = 'Brief comment about the given financial aid type entry.';
    ...
    {
        Schema::drop('financial_aid_types');
    }
}
